<?php
$header ="<header> 
<nav>
        <a href='index.php' class='navbar-brand'>Personal Portfolio </a>
        <ul id='main-navigation' class='navbar'>
            <li><a href='index.php' title='Home'>Home</a></li>
            <li><a href='about.php' title='about'>About Us</a></li>
            <li><a href='menu.php' title='menu'>Menu</a></li>
            <li><a href='locations.php' title='about'>Locations</a></li>
            <li><a href='contact.php' title='contact'>Contact Us</a></li>
        </ul>
    </nav>
</header>";

$footer = "<footer>  
<div class='frow'>
    <div class='col-3-md'>
            <h4 class='footer-list'>Navigation</h4>
        <ul class='footer-list'>    
            <li><a href='index.php' title='Home'>Home</a></li>
            <li><a href='about.php' title='about'>About Us</a></li>
            <li><a href='menu.php' title='menu'>Menu</a></li>
            <li><a href='locations.php' title='about'>Locations</a></li>
            <li><a href='contact.php' title='contact'>Contact Us</a></li>
        </ul>
    </div>
    <div class='col-3-md'>
        <h4 class='footer-list'>Lisa's Chicken</h4>
        <ul class='footer-list'>    
             <li>Hours</li>  
             <li>Mon-Thur: 9am-10pm</li> 
             <li>Saturday:9am-11pm</li>
             <li>Sunday:9am-10pm</li>
        </ul>
    </div>
    
    <div class='col-12-md'>
                <br> <br>
                <img class='icons' src='img/Instagram-09.svg' alt=''>
                <img class='icons' src='img/Twitter-01.svg' alt=''>
                <img class='icons' src='img/Facebook.svg' alt=''>
            </div> 
</div>
</footer>";
?>
